...
// Setting resetFloor to false allows the object to do things right on landing
function object_act_solid_gm(sides,top,bottom,resetActions, resetFloor=true, climbable=true)
{
						...
						if Player.Ysp >= 0 and !Player.OnObject
						{
							...
							with Player
							{
								...
								if !Grounded
								{
									Grounded = true;
									//if (resetFloor) 
									if (global.Character != CharMighty or (resetFloor and global.Character == CharMighty)) {
										PlayerResetOnFloor();
									}
								}
							}
